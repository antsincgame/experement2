...
> Pre-flight: `npm run check` **green** — agent **599+1 skip**, FE **126**. P0 CI: последние **5** прогонов `main` → **success** (включая `retries:2`).
...
| **P1 CI gate** | `web-project-deletion` добавлен в `playwright.e2e.ci.config.ts` (4 spec-файла) |
...
## Флаки-замер (P1 #3-4) — НЕ ЗАВЕРШЁН

Скрипт: `e2e/measure-flakiness.ps1` (5×6 excluded specs). Не запущен — e2e serial queue. Live-Metro спеки → **nightly only**.

...
const TEST_PROMPTS = [
  // Re-test failed from wave 2
  "Quote of the day app with share button",
  "Tip calculator with bill split",
  "Age calculator from birth date",
  // New prompts wave 3
  "Habit tracker with daily streak counter",
  "Mood journal with emoji selection",
  "Tic tac toe game for two players",
  "Currency converter with dropdown selectors",
...
const runTest = (testIndex) => {
  return new Promise((resolve) => {
    ...
    console.log(`\n${"=".repeat(60)}`);
    console.log(`${testName}: "${prompt.slice(0, 50)}..."`);
...
const main = async () => {
  ...
  for (let i = 0; i < TEST_PROMPTS.length; i++) {
    await runTest(i);
    // Delay between tests to avoid WS message overlap
    await new Promise((r) => setTimeout(r, 5000));
  }

  // Summary
  console.log(`\n${"=".repeat(60)}`);
  console.log(`RESULTS: ${passed} PASS / ${failed} FAIL / ${TEST_PROMPTS.length} TOTAL`);
  console.log(`${"=".repeat(60)}`);

...
const getTemplatePath = () => path.join(getWorkspaceRoot(), TEMPLATE_DIR_NAME);
export const initTemplateCache = async () => {
    if (cacheReady)
        return;
    if (cacheInitPromise)
...
export const createProjectFromCache = async (projectName, appDisplayName, extraDependencies) => {
    if (!cacheReady) {

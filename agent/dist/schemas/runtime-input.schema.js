...
const trimmedString = (fieldName, maxLength = 20_000) => z.string().trim()
...
const OptionalModelSchema = z.preprocess(emptyStringToUndefined, z.string().trim()
    .min(1, "model must not be empty")
    .max(200, "model is too long")
    .optional());
const safePathSegment = (fieldName) => trimmedString(fieldName, 120)
    .refine((value) => path.basename(value) === value, `${fieldName} must be a single path segment`)
    .refine((value) => value !== "." && value !== "..", `${fieldName} must not be a dot path`)
    .refine((value) => !value.includes("\0"), `${fieldName} contains invalid characters`);
...
//# sourceMappingURL=runtime-input.schema.js.map
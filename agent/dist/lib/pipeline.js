...
import { startExpo, startExpoClearCache, killExpo, getActivePort, runNativeSmoke, runTypecheck, runWebExport, npmInstall, } from "../services/process-manager.js";
...
const runProjectQualityGates = async (projectPath, navigationType) => {
    ...
    if (staticIssues.length > 0) {
        ...
        if (missingPkgIssues.length > 0) {
            const missingDeps = [...new Set(missingPkgIssues
                    .map((i) => {
            ...
            if (missingDeps.length > 0) {
                ...
                try {
                    ...
                    if (revalidated.length === 0) {
                        ...
                        errors.push(`Static validation failed: ${revalidated
        ...
        if (otherIssues.length > 0) {
            errors.push(`Static validation failed: ${otherIssues
                .map((issue) => `${issue.filePath ?? "project"}: ${issue.message}`)
                .join("; ")}`);
            return { success: false, errors };
        }
...
const _createProjectInner = async (options) => {
    const { description, lmStudioUrl, model, plannerModel, temperature, maxTokens, requestId, onProjectNameResolved, } = options;
    let projectSlug = null;
    const emitOperation = (message) => {
        broadcast({
            ...message,
            ...(requestId ? { requestId } : {}),
    ...
    const emitBuildScoped = (buildId, message) => {
    ...
    for (const file of [...plan.files]) {
        for (const dep of file.dependencies) {
            if (!dep.startsWith("src/") && !dep.startsWith("app/"))
                continue;
            if (planFilePaths.has(dep))
                continue;
    ...
    const files = await generateFiles({
        ...
        temperature,
        maxTokens,
        onFileStart: (filepath, index, total) => emitOperation({
            type: "file_generating",
            filepath,
            progress: (index + 1) / total,
        }),
        onChunk: (chunk) => emitOperation({ type: "code_chunk", chunk }),
        onFileComplete: (filepath) => emitOperation({ type: "file_complete", filepath }),
    });
    ...
    // ── Step 3b: Contract Validation + Auto-Fix ────────────
    {
        ...
        for (const fp of files) {
            ...
            while (retries <= MAX_CONTRACT_RETRIES) {
                ...
                const fixSuccess = await regenerateFileWithContracts(projectSlug, projectPath, fp, violations, allContracts, { lmStudioUrl, model, maxTokens });
                if (!fixSuccess) {
                ...
                if (updatedContracts)
                    allContracts[fp] = updatedContracts;
            }
        }
    }
    // ── Step 4: Git init ──────────────────────────────────
...
const _iterateProjectInner = async (options) => {
    ...
    const result = await editProject({
        ...
        onAnalysis: (action) => emitProject({
            type: "analysis_complete",
            files: action.files,
            thinking: action.thinking,
        }),
        onBlock: (block) => emitProject({ type: "block_applied", filepath: block.filepath, blockType: block.type }),
        onDiff: (filepath, before, after) => emitProject({
            type: "file_diff",
            filepath,
...
export const revertVersion = async (projectName, commitHash, _lmStudioUrl, requestId) => {
    const projectPath = getProjectPath(projectName);

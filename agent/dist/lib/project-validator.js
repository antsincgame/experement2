...
export const validateAppPlan = (plan) => {
    ...
    for (const file of plan.files) {
        for (const dependency of file.dependencies) {
            // Skip npm package names that LLM mistakenly puts in dependencies array
...
const extractStoreKeysFromSource = (projectPath, storePath) => {
    try {
        const fs = require("fs");
 ...
 */
export const autoHealImportContracts = (fileContent, contracts) => {
    ...
    for (const contract of allContracts) {
        if (contract.isDefaultExport)
            continue;
        const defaultPattern = new RegExp(`import\\s+${contract.name}\\s+from\\s+["']([^"']+)["']`, "g");
        result = result.replace(defaultPattern, (_match, modulePath) => `import { ${contract.name} } from "${modulePath}"`);
    }
    // Fix: import { X } from "path" → import X from "path" (when X is default export)
    for (const contract of allContracts) {
        if (!contract.isDefaultExport)
            continue;
        const namedPattern = new RegExp(`import\\s+\\{\\s*${contract.name}\\s*\\}\\s+from\\s+["']([^"']+)["']`, "g");
        result = result.replace(namedPattern, (_match, modulePath) => `import ${contract.name} from "${modulePath}"`);
    }
    return result;
};
/** Validate that generated files respect export contracts of their dependencies */
export const validateFileContracts = (fileContent, filePath, contracts) => {
    const violations = [];
    const allContracts = Object.values(contracts).flat();
    let match;
    ...
    while ((match = DESTRUCTURE_HOOK_REGEX.exec(fileContent)) !== null) {
        ...
        if (validKeys.length === 0) {
            ...
            const storeImportMatch = fileContent.match(new RegExp(`import.*${hookName}.*from\\s+["']([^"']+)["']`));
            if (storeImportMatch) {
                const importPath = storeImportMatch[1].replace("@/", "src/").replace(/^\.\//, "");
                const allFiles = Object.keys(contracts);
                const storeFile = allFiles.find((f) => f.includes(importPath.split("/").pop()?.replace(/\.ts$/, "") ?? ""));
                if (storeFile) {

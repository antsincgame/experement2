...
async function* toStream(text, chunkSize = 50) {
    for (let i = 0; i < text.length; i += chunkSize) {
        yield text.slice(i, i + chunkSize);
    }
}
async function* toLineStream(lines) {
...
describe("parseStream", () => {
    ...
    it("handles line-by-line streaming matching single-chunk output", async () => {
        ...
        const singleChunk = await collect(parseStream(toStream(lines.join(""), 1000)));
    ...
    it("handles mixed thinking + new file + SEARCH/REPLACE in one stream", async () => {
        ...
        expect(results[0]).toEqual({
            type: "thinking",
            content: "Creating a new utility and updating the main file.",
        });
        expect(results[1]).toEqual({
            filepath: "src/utils/helper.ts",
            type: "new_file",
    ...
    it("handles thinking with multiline content", async () => {
        const thinkingContent = "Line one.\nLine two.\nLine three with special chars: <>&\"'";
...
//# sourceMappingURL=stream-parser.test.js.map
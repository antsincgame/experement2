...
export const getRelevantDocs = (description, dependencies) => {
    const docs = [KNOWLEDGE_BASE.tamaguiCore];
    const text = (description + " " + dependencies.join(" ")).toLowerCase();
    if (text.match(/form|input|setting|switch|slider|check|radio/))
        docs.push(KNOWLEDGE_BASE.forms);
    if (text.match(/sheet|dialog|modal|overlay|popup|toast/))
        docs.push(KNOWLEDGE_BASE.overlays);
    if (text.match(/animat|bouncy|toggle|interactive|pressstyle|motion|smooth/))
        docs.push(KNOWLEDGE_BASE.animations);
    if (text.match(/alpine|x-data|x-show|x-transition|accordion|dropdown|modal web|micro-?interaction/))
        docs.push(KNOWLEDGE_BASE.alpineWeb);
    if (text.match(/tailwind|tailwindcss|hero|landing|marketing|template|dashboard|admin|card grid|pricing|auth form/))
        docs.push(KNOWLEDGE_BASE.tailwindTemplates);
    if (text.match(/sqlite|database|db|sql|offline/))
        docs.push(KNOWLEDGE_BASE.sqlite);
    if (text.match(/chart|stat|analytic|graph/))

// Shares safe search/replace matching so editor and autofix apply blocks consistently.
export interface SearchReplaceOutcome {
  result: string | null;
  error: string | null;
}
...
const fuzzyLineMatch = (contentLine: string, searchLine: string): boolean =>
  contentLine.trim() === searchLine.trim() ||
  normalizeLine(contentLine) === normalizeLine(searchLine);

const splitSearchLines = (search: string): string[] =>
  search
    .split("\n")
...
const adjustReplacementIndentation = (
  replace: string,
...
export const applySearchReplace = (
  ...
  const match =
    findFuzzyMatchRange(contentLines, searchLines) ??
  ...
  const adjustedReplace = adjustReplacementIndentation(
    replace,
    originalIndent,
    searchIndent
  );

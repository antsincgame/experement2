...
export type ShipFailureStage = "gates" | "metro" | "preview";

export interface CodegenShipResult {
  projectName: string;
  port: number;
  plan: AppPlan;
  /** True only when Metro bundle shipped and checkpoint reached `shipped`. */
  shipped: boolean;
  failureStage?: ShipFailureStage;
}
...
export interface RunCodegenShipParams {
  ...
  /** Skip LLM codegen — rerun gates + Metro only (ship-only resume). */
  skipCodegen?: boolean;
  gitCommitMessage?: string;
}

interface GenerateFilesPhaseParams {
  projectSlug: string;
  projectPath: string;
  plan: AppPlan;
  generationModel: string | undefined;
...
// Step 3: produce the project's source files — either by running LLM codegen, or (on a
// ship-only resume, skipCodegen) by listing the planned files already on disk. Emits
// progress and persists the codegen checkpoint.
const generateProjectFiles = async (params: GenerateFilesPhaseParams): Promise<string[]> => {
  const {
    projectSlug,
    projectPath,
    plan,
  ...
  if (skipCodegen) {
    const files = plan.files
      .map((file) => file.path)
      .filter((filePath) => !isAutoGeneratedPlanFile(filePath));
    emitOperation({
      type: "build_event",
      eventType: "ship_retry",
      message: "All planned files are on disk — rerunning quality gates and Metro preview.",
    });
    return files;
  }
...
interface TypeHealParams extends ContractHealParams {
  runTypecheck: PipelineContext["runTypecheck"];
}

// Step 3b: validate each generated file against the others' export contracts, auto-heal
// import mismatches with regex, and LLM-regenerate on remaining violations (bounded
// retries). Returns whether any repair ran (disqualifies the run from exemplar capture).
const healContractViolations = async (params: ContractHealParams): Promise<boolean> => {
  const { files, projectSlug, projectPath, fixModel, lmStudioUrl, maxTokens, complete, emitOperation } = params;
  ...
  return didContractFix;
};

// Step 3c: compiler-in-the-loop type-fix. Run the real typechecker, feed structured
// per-file errors back to the model, and iterate until the project compiles or we stop
// making progress. The fast inner loop BEFORE the heavy web-export / native gates.
// Returns whether any repair ran (disqualifies the run from exemplar capture).
const healTypeErrors = async (params: TypeHealParams): Promise<boolean> => {
  const { files, projectSlug, projectPath, fixModel, lmStudioUrl, maxTokens, complete, runTypecheck, emitOperation } = params;
...
interface BuildVerificationParams {
  buildId: string;
  projectSlug: string;
  projectPath: string;
  fixModel: string | undefined;
  lmStudioUrl?: string;
  complete: CompleteFn;
  startExpo: PipelineContext["startExpo"];
  runTypecheck: PipelineContext["runTypecheck"];
  emitOperation: (message: OutboundMessage) => void;
  emitBuildScoped: (buildId: string, message: OutboundMessage) => void;
}
...
// Step 6: start Metro, wait for the first bundle outcome, then run the bounded Metro
// autofix loop (each applied fix gated by a typecheck baseline so it can't regress the
// build). Returns the outcome + port so the orchestrator can run the post-fix gate and
// announce the preview.
const runBuildVerificationLoop = async (
  params: BuildVerificationParams,
): Promise<BuildVerificationResult> => {
  const { buildId, projectSlug, projectPath, fixModel, lmStudioUrl, complete, startExpo, runTypecheck, emitOperation, emitBuildScoped } = params;
  ...
  void triggerMetroBuild(expoPort).catch((error) => {
    warnCaught("pipeline-codegen-phase", error, "triggerMetroBuild during build verification");
  });
  ...
  while (buildError && autoFixAttempts < MAX_BUILD_AUTOFIX) {
    ...
    try {
      ...
      warnCaught("pipeline-codegen-phase", error, "autofix baseline typecheck failed");
      baselineErrors = 0;
    }
  ...
  return { buildSuccess, buildError, autoFixAttempts, expoPort };
};

interface QualitySignalsParams {
  files: string[];
  projectSlug: string;
  plan: AppPlan;
  autoFixAttempts: number;
  didContractFix: boolean;
  didTypeFix: boolean;
  complete: CompleteFn;
  fixModel: string | undefined;
  generationModel: string | undefined;
...
// Quality score + accretive-memory capture (Phase 1 + Phase 3). Called ONLY on a
// verified-ready build; computes the deterministic quality score ONCE and uses it to
...
const captureQualitySignals = async (params: QualitySignalsParams): Promise<void> => {
  const {
    files,
    projectSlug,
    plan,
  ...
  const cleanGeneration = autoFixAttempts === 0 && !didContractFix && !didTypeFix;
...
export const runCodegenAndShip = async (
  ...
  // Anti-regression snapshot for the WHOLE repair phase (3b contract-fix + 3c type-fix).
  // Capture each plan file's content BEFORE any repair runs; the gate below restores them
  // if the repair phase left the project typechecking WORSE than it started. Cheap:
  // in-memory reads now, at most two extra `tsc` runs later (only if a repair changed a file).
  ...
  // ── Step 3b/3c: Repair phase ──────────────────────────
  // didContractFix/didTypeFix gate the learned-exemplar capture: any repair flips a flag
  // and disqualifies the run from being used as teaching material (quality-drift guard).
  const repairDeps = { files, projectSlug, projectPath, fixModel, lmStudioUrl, maxTokens, complete, emitOperation };
  const didContractFix = await healContractViolations(repairDeps);
  const didTypeFix = await healTypeErrors({ ...repairDeps, runTypecheck });
  ...
  const build = await runBuildVerificationLoop({
    buildId,
    projectSlug,
    projectPath,
    fixModel,
  ...
  let buildSuccess = build.buildSuccess;
  let buildError = build.buildError;
  const autoFixAttempts = build.autoFixAttempts;
  const expoPort = build.expoPort;
  ...
  const failureStage: ShipFailureStage | undefined = shipped
    ? undefined
    : buildError?.includes("Metro")
      ? "metro"
      : buildError
        ? "gates"
        : "preview";
  ...
  // Best-effort: nothing here can affect the already-shipped result. See
  // captureQualitySignals for the capture-tier rationale.
  if (buildSuccess) {
    await captureQualitySignals({
      files,
      projectSlug,
      plan,
      autoFixAttempts,
  ...
  return {
    ...
    plan,
    shipped,
    failureStage: shipped ? undefined : failureStage,
  };
};

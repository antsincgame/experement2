// Plan brief: primary LLM-facing narrative; JSON on disk is the machine contract.
 ...
 */
export const formatPlanBriefForModels = (plan: AppPlan): string => {
  const theme = plan.theme;
  const nav = plan.navigation;
  const screens = plan.files.filter((f) => f.type === "screen");
  const components = plan.files.filter((f) => f.type === "component");
  const stores = plan.files.filter((f) => f.type === "store");
  const hooks = plan.files.filter((f) => f.type === "hook");
  const types = plan.files.filter((f) => f.type === "type");
  const other = plan.files.filter(
  ...
  for (const screen of nav?.screens ?? []) {
    lines.push(`- **${screen.name}** → \`${screen.path}\`${screen.icon ? ` (icon: ${screen.icon})` : ""}`);
  }
  ...
  lines.push(
    "",
    "## Build map",
    `${plan.files.length} files total. Follow this narrative first; \`.appfactory/blueprint.json\` is the exact path/dependency graph.`,
    "",
  );
...
export const formatPlanBriefForChat = (plan: AppPlan): string => {
  ...
  if (components.length > 0) scope.push(`**${components.length}** component${components.length > 1 ? "s" : ""}`);
  ...
  lines.push(
    "",
    `**${plan.files.length} files** in the build queue.`,
    "",

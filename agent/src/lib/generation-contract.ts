...
/** Remove scaffold-written layouts from plan.files and dependency lists. */
export const stripAutoGeneratedPlanFiles = (plan: {
  files: Array<{ path: string; dependencies: string[] }>;
}): void => {
  const autoPaths = new Set<string>(AUTO_GENERATED_PLAN_FILES);
  plan.files = plan.files.filter((file) => !autoPaths.has(file.path));
  for (const file of plan.files) {
    file.dependencies = file.dependencies.filter((dep) => !isAutoGeneratedPlanFile(dep));
  }
};

/** Strip auto-layouts; returns true when the plan was mutated (for disk migration). */
export const stripAutoGeneratedPlanFilesIfNeeded = (plan: {
  files: Array<{ path: string; dependencies: string[] }>;
}): boolean => {
  const before = JSON.stringify(
    plan.files.map((file) => ({ path: file.path, dependencies: file.dependencies })),
  );

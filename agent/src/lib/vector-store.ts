...
/** True only when every component is a finite number (rejects NaN/Infinity). */
...
export const cosineSimilarity = (a: number[], b: number[]): number => {
  ...
  // A NaN/Infinity score (e.g. an Infinity component) would poison the top-K
  // comparator (b.score - a.score), so collapse it to a non-match.
...
export const searchTopK = (
  ...
  // A non-finite query vector cannot be compared meaningfully against anything.
  if (!isFiniteVector(queryVector)) return [];

  const scored: ScoredChunk[] = [];
  for (const chunk of chunks) {
    if (chunk.vector.length !== queryVector.length) continue;
    if (!isFiniteVector(chunk.vector)) continue;
